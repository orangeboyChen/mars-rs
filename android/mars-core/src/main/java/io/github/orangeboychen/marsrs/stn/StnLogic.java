/**
 *  The Java face of STN: the `native` methods are the
 *  `Java_io_github_orangeboychen_marsrs_stn_StnLogic_*` symbols of `mars-jni`,
 *  and the private static methods below are the ones it calls back — the
 *  port of `com_tencent_mars_stn_StnLogic_C2Java.cc`.
 *
...
public class StnLogic {
    ...
    public interface ICallBack {
         ...
         * @return
         */
        boolean req2Buf(final int taskID, Object userContext, ByteArrayOutputStream reqBuffer, int[] errCode, int channelSelect, final String host, final int sequence);
         ...
         * @return  int
         */
        int buf2Resp(final int taskID, Object userContext, final byte[] respBuffer, int[] errCode, int channelSelect, final int[] sequence);
     ...
     */
    public static native void resetAndInitEncoderVersion(int packerEncoderVersion, final String packerEncoderName);
    ...
    /**
     * 一个随机的 seq, 与 C++ 的 `unsigned short` 相同 — 端口有而 C++ 的 Java
     * 类没有声明。
     */
    public static native int genSequenceId();

    /**
     * 触发一次 noop — 端口有而 C++ 的 Java 类没有声明。
     */
    public static native void trigNooping();
    ...
    private static boolean req2Buf(final int taskID, Object userContext, ByteArrayOutputStream reqBuffer, int[] errCode, int channelSelect, final String host, final int sequence) {
        try {
            if (callBack == null) {
                new NullPointerException("callback is null").printStackTrace();
                return false;
            }
            return callBack.req2Buf(taskID, userContext, reqBuffer, errCode, channelSelect, host, sequence);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return false;
     ...
     * @return
     */
    private static int buf2Resp(final int taskID, Object userContext, final byte[] respBuffer, int[] errCode, int channelSelect, final int[] sequence) {
        try {
            if (callBack == null) {
                new NullPointerException("callback is null").printStackTrace();
                return RESP_FAIL_HANDLE_TASK_END;
            }
            return callBack.buf2Resp(taskID, userContext, respBuffer, errCode, channelSelect, sequence);
        } catch (Exception e) {
            e.printStackTrace();
        }
