...
public class Alarm extends BroadcastReceiver {
    ...
    // No `WakerLock` here: the port takes the wake lock itself, on the thread
    // that heard the alarm (`mars-jni`'s `START_ALARM_WAKELOCK_MS`), which is
    // where the C++ takes it too and the only place acquiring one works.
