...
layers of Mars talk to `mars-appender` **without** a big-bang rewrite.
...
cargo build -p mars-ffi --release
```
...
```sh
cargo build -p mars-ffi --release --target aarch64-linux-android
cargo build -p mars-ffi --release --target aarch64-apple-ios
```
...
```sh
# static
clang log_bridge.c -I rust/crates/mars-ffi/include \
